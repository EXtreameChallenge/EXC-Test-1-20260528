...
export function connectWebSocket(onConnected, onError) {
  ...
  try {
    ...
    stompClient.connect(headers, (frame) => {
      isConnected = true
      console.log('[WS-PC] WebSocket connected')
      ...
      stompClient.subscribe('/topic/vehicle', (message) => {
        try {
          ...
          console.error('[WS-PC] Parse vehicle update error:', e)
        }
      })
      ...
      stompClient.subscribe('/topic/alert', (message) => {
        try {
          ...
          console.error('[WS-PC] Parse alert update error:', e)
        }
      })
      ...
      stompClient.subscribe('/topic/task', (message) => {
        try {
          ...
          console.error('[WS-PC] Parse task update error:', e)
        }
      })
      ...
      isConnected = false
      console.error('[WS-PC] Connection error:', error)
      if (onError) onError(error)
      scheduleReconnect(onConnected, onError)
    })
  } catch (e) {
    console.error('[WS-PC] Failed to create WebSocket:', e)
...
function scheduleReconnect(onConnected, onError) {
  ...
  reconnectTimer = setTimeout(() => {
    console.log('[WS-PC] Attempting reconnect...')
...
function emit(event, data) {
  ...
  listeners[event].forEach(cb => {
    try {
      ...
      console.error(`[WS-PC] Listener error for ${event}:`, e)
    }
  })
}
