...
app.post('/api/chat', async (req, res) => {
  ...
  try {
    ...
    if (err.name === 'AbortError') {
      res.status(504).json({ error: 'API请求超时（30秒）' });
    } else {
...
app.get('/api/health', (req, res) => { res.json({ status: 'ok', demo: '运行中', springBoot: 'pending', timestamp: new Date().toISOString() }); });
...
server.listen(PORT, () => {
  console.log(`轻行Claw代理服务器运行在 http://localhost:${PORT}`);
