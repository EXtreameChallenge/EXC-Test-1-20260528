...
```
PC端 Vue (:5173) ──┐
                     └─> Node.js 代理服务器 (:3000) ──> Spring Boot 后端 (:8080)
移动端 Demo ────────┘
```

**核心思路**：
1. Node.js 代理服务器 (`server.js`) 作为统一入口
2. 移动端 demo.html 和 PC 端 Vue 都通过代理访问后端
3. 代理同时提供 Mock 数据和 GLM API 代理
4. PC 端改用 5173 端口，避免与 Node 代理 3000 端口冲突

---

## 二、连接步骤

### 步骤 1：修改 PC 端 Vite 配置

打开文件：`C:\Users\Challenge\Desktop\vue-login-app20260426\vite.config.js`
...
**关键变更**：
- `port: 3000` → `port: 5173`
- `target: 'http://localhost:8080/JavaEEWeb03'` → `target: 'http://localhost:3000'`

---

### 步骤 2：更新 Node.js 代理服务器

打开文件：`C:\Users\Challenge\WorkBuddy\20260423122826\server.js`

将内容替换为 `server-enhanced.js` 中的内容（已在下方提供）。

---

### 步骤 3：创建共享 Mock 数据

在项目根目录创建 `mock-data.js`，内容见下方。
...
```bash
# 1. 启动 Node.js 代理（包含 demo.html + API 代理 + Mock 数据）
cd C:\Users\Challenge\WorkBuddy\20260423122826
node server.js

# 2. 启动 PC 端 Vue 项目
cd C:\Users\Challenge\Desktop\vue-login-app20260426
npm run dev
```
...
| 服务 | 地址 | 说明 |
|------|------|------|
| 移动端 Demo | http://localhost:3000 | 移动优先，手机端访问 |
| PC 端管理后台 | http://localhost:5173 | Vue3 后台管理系统 |
| API 代理 | http://localhost:3000/api | 统一 API 入口 |
| Spring Boot 后端 | http://localhost:8080 | Java 后端（如果已启动） |

---

## 四、文件内容

### server-enhanced.js（完整代理服务器）
  ...
  try {
    const response = await fetch('https://open.bigmodel.cn/api/paas/v4/chat/completions', {
      ...
      headers: {
        ...
        'Authorization': `Bearer 70888881134e4137aa2956b6534d6ac6.X83pGlMih9mmmlxq`
      },
...
app.use('/api/v1', (req, res) => {
  const target = `http://localhost:8080/JavaEEWeb03${req.originalUrl}`;
  fetch(target, {
    method: req.method,
    headers: { ...req.headers },
...
app.get('/api/dispatch', (req, res) => {
  res.json({
    code: 200,
    data: { content: mockData.tasks, totalElements: mockData.tasks.length }
  });
});
...
打开文件：`C:\Users\Challenge\Desktop\vue-login-app20260426\src\api\index.js`
...
export function batchOperateVehicles(vehicleIds, action) {
  return api.post('/vehicles/batch', { vehicleIds, action })
}

// ===== 任务接口 =====
export function getTasks(params = {}) {
  return api.get('/dispatch', { params })
}
...
export function confirmTask(id) {
  return api.post(`/dispatch/${id}/confirm`)
}

// ===== 运营分析接口 =====
export function getDailyAnalytics(params = {}) {
  return api.get('/analytics/daily', { params })
}

export function getEnergyStats(params = {}) {
  return api.get('/energy/stats', { params })
}
...
import { getVehicles, getVehicleDetail, batchOperateVehicles } from '../api'
...
export const useVehicleStore = defineStore('vehicle', {
  ...
  actions: {
    ...
    async batchOperate(vehicleIds, action) {
      try {
        await batchOperateVehicles(vehicleIds, action)
        this.fetchVehicles()
      } catch (err) {
...
### src/stores/auth.js (TypeScript → JavaScript)

由于你的项目是 `.js` 而非 `.ts`，请将 `auth.ts` 重命名为 `auth.js`：
  ...
  actions: {
    async login(username, password) {
      try {
        ...
        this.token = res.data?.token || 'demo-token'
        this.user = res.data?.user || { name: username, role: '运营管理员' }
        ...
        this.user = { name: username, role: '运营管理员', permissions: ['车队管理', '任务调度', '能源管理', '运营分析'] }
        localStorage.setItem('auth_token', this.token)
        localStorage.setItem('user', JSON.stringify(this.user))
        return true
      }
...
| 测试 | 地址 | 预期结果 |
|------|------|---------|
| 移动端 | http://localhost:3000 | demo.html 正常加载 |
| PC 端 | http://localhost:5173 | Vue 登录页正常加载 |
| 车辆 API | http://localhost:3000/api/vehicles | JSON 数据返回 |
| 运营 API | http://localhost:3000/api/analytics/daily | JSON 数据返回 |
